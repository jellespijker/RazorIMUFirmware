...
void Accel::read() {
    ...
    if (request(buffer, 6, 0x32)) {
        xyz_[0] = ((int16_t) ((((uint16_t) buffer[3]) << 8) | buffer[2])) * mg;  // X axis (internal sensor y axis)
        xyz_[1] = ((int16_t) ((((uint16_t) buffer[1]) << 8) | buffer[0])) * mg;  // Y axis (internal sensor x axis)
